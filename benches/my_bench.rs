...
impl Computer {
    ...
    fn get_arguments_old(&mut self, index: &usize, mut opcode: isize) -> Vec<usize> {
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench(
        "routines",
        Benchmark::new("Sum primes", |b| b.iter(|| sum_of_primes())).sample_size(10),
    );
    c.bench(
        "routines",
        Benchmark::new("Prime small", |b| b.iter(|| prime_factor_small())).sample_size(10),
    );
    c.bench(
        "routines",
        Benchmark::new("Prime big}", |b| b.iter(|| prime_factor_big())).sample_size(10),
    );
    /*
